...
<table class="table table-bordered table-hover">
 ...
 while ($row = mysqli_fetch_assoc($selectComments)) {
   ...
   echo "<td>{$comment_content}</td>"; //  $query = "SELECT * FROM categories WHERE cat_id = {$post_category_id} "; //  $selectCategoriesId = mysqli_query($connection, $query); //  while ($row = mysqli_fetch_assoc($selectCategoriesId)) { //      $cat_id = $row['cat_id']; //      $cat_title = $row['cat_title']; //  }
   ...
   echo "<td><a href='post_comments.php?delete=$comment_id&id=" .
     $_GET["id"] .
     "'>Delete</a></td>"; //  echo "<td><a href='posts.php?source=edit_post&p_id='>Edit</a></td>";
   echo "</tr>";
 }
 ?>
